...
export function AnalyticsView() {
  ...
  const rawTopFiles = data?.top_files || data?.most_viewed_files || [];
  const recentActivity = data?.recent_activity || [];

  const topFilesList = rawTopFiles.map((f) => ({
    file_id: f.id || f.file_id || "1",
    file_name: f.file_name || f.name || "kibi.jpg",
    value: topFilesMetric === "views" ? (f.total_views ?? f.views ?? 0) : (f.total_downloads ?? f.downloads ?? 0)
  }));

  return (
    <div className="space-y-6 text-[#C5C3C4] relative animate-fade-in analytics-theme p-6">
      {/* Top Action Bar */}
                ...
                {recentActivity.length === 0 ? (
                  ...
                  <div className="space-y-2.5">
                    {recentActivity.map((log, index) => {
                      ...
                      const rawUrl = log.share_url || `/share/${log.id || ""}`;
                      const fileLink = rawUrl.includes(":8000") ? `/share/${log.id || ""}` : rawUrl;

                      return (
                        <div
                          key={index}
                          className="p-3 bg-[#1E1F2B] border border-[#34364A] rounded-xl flex items-center justify-between text-xs hover:border-[#7C5CFC]/40 transition-colors"
                        >
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="text-gray-400 text-[11px] flex items-center gap-2 mt-0.5">
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="font-semibold text-[#7C5CFC] hover:underline flex items-center gap-1 cursor-pointer"
                                >
                                  View File ↗
                                </a>
                                <span>•</span>
                                <span>{formatDate(log.timestamp || log.created_at)}</span>
                              </div>
                            </div>
                          </div>
