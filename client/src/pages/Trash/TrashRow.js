    ...
    <tr className="border-b border-slate-800 hover:bg-slate-800 transition-colors duration-200">
      <td className="px-6 py-4">
        <div className="flex flex-col">
          <span className="font-semibold text-white">{file.original_name}</span>
          <span className="text-xs text-gray-400 mt-1">{file.file_name}</span>
        </div>
      </td>

      <td className="px-6 py-4">
        <span className="text-gray-300 uppercase">{file.file_extension || "-"}</span>
      </td>
