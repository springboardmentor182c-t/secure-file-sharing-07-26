...
SMTP_FROM = os.getenv("SMTP_FROM", SMTP_USER or "no-reply@trustshare.local")
...
def send_email(to: str, subject: str, body: str) -> bool:
    """Sends a plain-text email over SMTP. Returns False (never raises) if
    sending fails, so callers can fall back to a dev-mode response."""
...
def request_email_otp(payload: EmailOtpRequestIn, db: Session = Depends(get_db)):
    ...
    if email_is_configured():
        sent = send_email(
            ...
            f"Your verification code is {code}. It expires in {EMAIL_OTP_MIN} minutes.",
        )
        if sent:
            return EmailOtpRequestOut(message=f"A code was sent to {user.email}")
        # SMTP was configured but sending failed — fall through to dev mode.

    # No email server is configured (or sending failed), so the code is
    # returned directly for local testing instead of being sent.
...
def forgot_password(payload: ForgotPasswordIn, db: Session = Depends(get_db)):
    ...
    if email_is_configured():
        sent = send_email(
            user.email,
            "Reset your TrustShare password",
            f"Click the link below to reset your password. This link expires in 15 minutes.\n\n{reset_link}",
        )
        if sent:
            return ForgotPasswordOut(message=generic)
        # SMTP was configured but sending failed — fall through to dev mode so the
        # flow doesn't silently dead-end.
