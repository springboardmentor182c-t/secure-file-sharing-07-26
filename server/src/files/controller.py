...
async def upload_file(
    ...
    folder_id: Optional[Union[str, int]] = Form(default=None),
    ...
    if category:
        cat_str = str(category).strip().capitalize()
    ...
    file_obj = service.upload_file(
        ...
        contents=contents, folder_id=clean_folder_id, category=clean_category,
    )
...
def list_files(
    ...
    folder_id: Optional[Union[str, int]] = Query(default=None),
    ...
    files, total = service.search_files(
        db, owner_id=owner_id, search=search, category=category, folder_id=clean_folder_id,
        starred_only=starred, trashed_only=False, sort_by=SortField.NEWEST, page=page, page_size=page_size,
    )
...
@router.delete("/trash", response_model=ApiResponse[None], summary="Empty all files in Trash")
def empty_trash_endpoint(owner_id: Annotated[uuid.UUID, Depends(get_current_user_id)], db: Annotated[Session, Depends(get_db)]):
    service.empty_trash(db, owner_id=owner_id)
    return ApiResponse(message="Trash emptied", data=None)
...
def download_file(file_id: Union[str, int], owner_id: Annotated[uuid.UUID, Depends(get_current_user_id)], db: Annotated[Session, Depends(get_db)]):
...
def rename_file(
    file_id: Union[str, int], payload: FileRenameRequest,
...
def move_file(
    file_id: Union[str, int], payload: FileMoveRequest,
    owner_id: Annotated[uuid.UUID, Depends(get_current_user_id)], db: Annotated[Session, Depends(get_db)],
):
...
def set_category(
    file_id: Union[str, int], payload: FileCategoryUpdateRequest,
    owner_id: Annotated[uuid.UUID, Depends(get_current_user_id)], db: Annotated[Session, Depends(get_db)],
):
...
def permanently_delete_file(file_id: Union[str, int], owner_id: Annotated[uuid.UUID, Depends(get_current_user_id)], db: Annotated[Session, Depends(get_db)]):
...
def list_folders(
    owner_id: Annotated[uuid.UUID, Depends(get_current_user_id)], db: Annotated[Session, Depends(get_db)],
    parent_id: Optional[Union[str, int]] = Query(default=None),
):
...
def rename_folder(
    folder_id: Union[str, int], payload: FolderRename,
    owner_id: Annotated[uuid.UUID, Depends(get_current_user_id)], db: Annotated[Session, Depends(get_db)],
):
...
def delete_folder(folder_id: Union[str, int], owner_id: Annotated[uuid.UUID, Depends(get_current_user_id)], db: Annotated[Session, Depends(get_db)]):
