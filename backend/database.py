...
def get_all_files():
    """Retrieves all stored files, sorted by upload date descending."""
    with get_db_connection() as conn:
        cursor = conn.cursor()
...
def get_files_by_category(category: str):
    ...
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM files WHERE category = ? ORDER BY upload_date DESC", (category,))
        rows = cursor.fetchall()
        return [dict(row) for row in rows]
...
def get_duplicate_logs():
    """Retrieves logs of duplicate prevention activities, sorted by timestamp descending."""
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM duplicate_logs ORDER BY timestamp DESC")
        rows = cursor.fetchall()
        return [dict(row) for row in rows]

def get_dashboard_stats() -> dict:
    """Computes statistics for the dashboard dashboard."""
    with get_db_connection() as conn:
        cursor = conn.cursor()
