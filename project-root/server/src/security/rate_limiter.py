...
# RATE LIMITER (In-Memory with DB Config)
...
class RateLimiter:
    ...
    def _get_config(self, endpoint: str, db=None) -> dict:
        """Get rate limit config from DB or defaults."""
        # Try DB first
        if db:
            try:
                now = time.time()
                if not self._config_cache or (now - self._config_timestamp) > self._CONFIG_TTL:
                    from src.security.models.app_config import AppConfig
                    import json
                    ...
                    if config_row and config_row.value:
                        self._config_cache = json.loads(config_row.value)
    ...
    def reset(self, client_id: Optional[str] = None) -> None:
        ...
        with self._lock:
            if client_id:
                keys_to_remove = [
                    k for k in self._requests if k.startswith(f"{client_id}:")
                ]
