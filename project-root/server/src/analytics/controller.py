...
# FIX ISS-10: Removed duplicate StreamingResponse import
...
def _resolve_days_from_custom(
    ...
    """
    If custom dates provided, calculate actual days.
    Otherwise return the provided `days` value.

    FIX ISS-5: Changed cap from 365 to 3650 to match
    the Query parameter limit (le=3650).
    """
    if start_date and end_date:
        try:
            ...
            return max(1, min(3650, actual_days))   # FIX: was min(365, ...)
        except (ValueError, TypeError):
            return days
    return days
...
def export_file_analytics_pdf(
    ...
    date_label  = (
        _custom_date_range_label(start_date, end_date)
...
def export_csv(
    ...
    if tab == "security":
        ...
        # FIX ISS-7 applied: use correct dict keys from serialized activities
        writer.writerow(["=== RECENT ACTIVITY ==="])
        writer.writerow(["Event", "User", "File", "Time"])  # ← cleaner headers
        ...
        for activity in activities[:50]:
            ...
            user = (
                ...
                or (f"User #{activity.get('user_id')}" if activity.get("user_id") else "System")
            )
            file = (
                activity.get("file_name")
                or (f"File #{activity.get('file_id')}" if activity.get("file_id") else "")
            )
            writer.writerow([
                activity.get("event_type", ""),
                user,
                file,
                activity.get("time", "") or activity.get("created_at", ""),
            ])
    ...
    # ═══════════════════════════════════════════════════════
        ...
        writer.writerow(["Metric", "Value"])
        writer.writerow(["Storage Used",             f"{storage_data.get('storage_used_gb',   0):.2f} GB"])
        writer.writerow(["Storage Quota",            f"{storage_data.get('storage_quota_gb',  0)} GB"])
        writer.writerow(["Storage Percentage",       f"{storage_data.get('storage_percentage',0)}%"])
        writer.writerow(["Files Uploaded (period)",  uploads_data.get("total_uploads",         0)])
        writer.writerow(["Uploads Today",            uploads_data.get("today_uploads",         0)])
        writer.writerow(["Total Downloads (period)", downloads_data.get("total_downloads",     0)])
        ...
        writer.writerow(["Files Deleted (period)",   deletes_data.get("total_deletes",         0)])
        ...
        # FIX ISS-7 applied: use correct dict keys from serialized activities
        writer.writerow(["=== RECENT ACTIVITY ==="])
        writer.writerow(["Event", "User ID", "File ID", "Time"])
        recent_data = data.get("recent_activity", {}) or {}
        activities  = recent_data.get("activities") or []
        for activity in activities[:50]:
            writer.writerow([
