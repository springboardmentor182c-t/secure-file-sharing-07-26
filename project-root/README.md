...
application metadata, while the configured persistent storage protects encrypted
file payloads. Per-file AES keys are wrapped by a separate production master key.
...
| `FRONTEND_URL` | No | Public HTTPS application URL |
| `BACKEND_URL` | No | Public HTTPS API URL |
